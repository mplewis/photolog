...
export function toDate(
  ...
  | { date: Date; localDate: LocalDate; success: true }
  | { error: string; success: false } {
  const dtoRe = /^(\d{4}):(\d{2}):(\d{2}) (\d{2}):(\d{2}):(\d{2})$/;

  const dtoMatch = dto.match(dtoRe);
  if (!dtoMatch)
    return { error: "Could not parse DateTimeOriginal field", success: false };

  const [, year, month, day, hour, minute, second] = dtoMatch as [
    any,
    string,
    string,
    string,
 ...
 */
export function parseExiftoolMetadata(raw: string): Metadata {
  ...
  if (Object.keys(kv).length === 0)
    throw new Error("Could not parse any metadata");

  const data = metadataSchema.parse(kv);
  ...
  if (data.DateTimeOriginal) {
    ...
    if (r.success) {
      ...
      console.warn(
        `Error parsing date from ${data.DateTimeOriginal}, ${data.OffsetTimeOriginal}: ${r.error}`
      );
    }
  }
  ...
  if (
    data.Location &&
    data.Sublocation &&
    data.Sublocation.length > data.Location.length
  ) {
